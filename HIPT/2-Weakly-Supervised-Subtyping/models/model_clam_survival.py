...
class CLAM_SB(nn.Module):
    def __init__(self, path_input_dim=1024, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        instance_loss_fn=nn.CrossEntropyLoss(), feature_type='beph', subtyping=False):
        ...
        if(feature_type =='beph' or feature_type =='ctranspath'):
            self.feature_linear1 = nn.Linear(768, 384)
        elif(feature_type =='resnet' ):
            self.feature_linear1 = nn.Linear(1024, 384)
        self.classifiers = nn.Linear(size[1], n_classes)
        self.risk_score = nn.Linear(2, 1)
    ...
    def relocate(self):
        ...
        if(self.feature_type in ['beph','ctranspath','resnet']):
            self.feature_linear1 = self.feature_linear1.to(device)
        # self.feature_linear2 = self.feature_linear2.to(device)
        self.attention_net = self.attention_net.to(device)
        self.classifiers = self.classifiers.to(device)
        self.risk_score = self.risk_score.to(device)
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False, **kwargs):
        device = h.device
        if(self.feature_type in ['beph','ctranspath','resnet']):
            h = self.feature_linear1(h.float())
