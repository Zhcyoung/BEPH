...
def collate_MIL(batch):
    ...
    # y_time = torch.LongTensor([item[2] for item in batch])
    # y_event = torch.LongTensor([item[3] for item in batch])
    # return [img,label,y_time,y_event]
    ####========
    img = torch.cat([torch.tensor(item[0]) for item in batch], dim = 0)
    label = torch.LongTensor([0])
    event_time = torch.LongTensor([item[2] for item in batch])
    c = torch.LongTensor([item[3] for item in batch])
