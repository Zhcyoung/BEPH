...
from utils.utils_survival_mcat import *
import os
import torch.nn.functional as F
...
def train_loop_clam(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None, dropinput=0.0):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        # print('================',label)
        logits, Y_prob, Y_hat, _, instance_dict = model(h=data, cluster_id=cluster_id,y_time=y_time,y_event=y_event, label=label, instance_eval=True)
...
###============
        ...
        if(step!=0 and step%10 ==0):
                ...
                total_loss = bag_weight * coxloss + (1-bag_weight) * instance_loss 

                epochCoxloss += coxloss.item()+instance_loss_value
                ...
                if(gc!=0 and gc%20 == 0):
                    optimizer.step()
                    optimizer.zero_grad()
    epochCoxloss = epochCoxloss*10/len(loader)
