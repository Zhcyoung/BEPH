...
class EarlyStopping:
    ...
    def __init__(self, patience=20, stop_epoch=50, verbose=False):
        ...
        self.c_index_min = np.Inf

    def __call__(self, epoch, c_index, model, ckpt_name = 'checkpoint.pt'):

        score = c_index

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(c_index, model, ckpt_name)
    ...
    def save_checkpoint(self, c_index, model, ckpt_name):
        '''Saves model when validation loss decrease.'''
        if self.verbose:
            print(f'Validation loss decreased ({self.c_index_min:.6f} --> {c_index:.6f}).  Saving model ...')
        torch.save(model.state_dict(), ckpt_name)
        self.c_index_min = c_index
...
###============
        ...
        if(step!=0 and step%10 ==0):
                ...
                if(gc!=0 and gc%bt == 0):
                    optimizer.step()
                    optimizer.zero_grad()
    epochCoxloss = epochCoxloss*bt/len(loader)
...
def validate_clam(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir = None):
    ...
    if early_stopping:
        assert results_dir
        early_stopping(epoch, cindex, model, ckpt_name = os.path.join(results_dir, "s_{}_checkpoint.pt".format(cur)))
