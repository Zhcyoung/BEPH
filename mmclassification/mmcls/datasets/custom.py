...
#             info = {'img_path': img_path, 'gt_label': int(gt_label),'hello_world':int(3)}
...
#         return data_list
    ...
    def load_data_list(self):
        ...
        if(len(samples[0])>2):
            ...
            backend = get_file_backend(self.img_prefix, enable_singleton=True)
            data_list = []
            for (filename,gt_label,y_time,y_event) in samples:
                img_path = backend.join_path(self.img_prefix, filename)
                info = {'img_path': img_path, 'gt_label': int(gt_label),'y_time':int(y_time),'y_event':int(y_event)}
                data_list.append(info)
        if(len(samples[0])==2):
