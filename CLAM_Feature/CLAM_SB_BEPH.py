...
def main(args):
    ...
    for i in folds:
        ...
        train_dataset, val_dataset, test_dataset = dataset.return_splits(from_id=False, csv_path='{}/splits_{}.csv'.format(args.splits, i))
...
Label_csv = pd.read_csv(args.csv_path)['label']
label_dict = {value: index for index, value in enumerate(np.unique(Label_csv))}

if args.task == 'tcga_lung_subtype':
    args.n_classes = 2
    dataset = Generic_MIL_Dataset(csv_path = '../dataset_csv/tcga_lung_subset.csv',
    ...
    args.n_classes = 3
    dataset = Generic_MIL_Dataset(csv_path = '../dataset_csv/tcga_kidney_subset.csv',
                            data_dir= os.path.join(args.data_root_dir, study_dir),
                            mode=args.mode,
                            shuffle = False, 
                            seed = args.seed, 
